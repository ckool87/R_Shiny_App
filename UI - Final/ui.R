  ...
  tabsetPanel(
    ...
    tabPanel("Survey Questions",
             ...
             wellPanel(
               ...
               uiOutput("vehicleInputs"),  # This will dynamically generate the UI for vehicle inputs,
             ),
             ...
             wellPanel(
               fluidRow(
                 column(6, radioButtons("f45", "Purchase green power?",
                        ...
                        helpText("Reference: F45")),
                 column(6, numericInput("f49", "Percentage of green power", value = 50),
                        helpText("Reference: F49"))
               ),
               fluidRow(
                 column(6, numericInput("f53", "Fuel oil usage (monthly)", value = 50),
                        helpText("Reference: F53")),
                 column(6, radioButtons("h53", "Billing unit for fuel oil",
                                        choices = c("Dollars" = 1, "Gallons" = 2)),
                        helpText("Reference: H53")),
                 column(6, numericInput("f57", "Propane usage (monthly)", value = 50),
                        helpText("Reference: F57")),
                 column(6, radioButtons("h57", "Billing unit for propane",
                                        choices = c("Dollars" = 1, "Gallons" = 2)),
                        helpText("Reference: H57"))
               )
             ),
             fluidRow(
               class="bg-red",
               div(style = "display: flex; flex-wrap: wrap;", # This will allow the contents to wrap in case of a narrow screen
                   div(style = "flex: 50%; padding: 5px;", # This sets each div to take up half the horizontal space
